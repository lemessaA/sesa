...
import apiService, { getAxiosErrorMessage } from '../utils/api';
    ...
    const generateAIResponse = async (
        ...
        if (token) {
            try {
                ...
                console.warn('[AIHelper] ai-agent chat failed', err);
                if (useRag) {
                ...
                {isOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed inset-0 z-50 flex w-full max-w-none flex-col overflow-hidden bg-white sm:inset-auto sm:bottom-5 sm:right-5 sm:left-auto sm:top-auto sm:h-[min(720px,calc(100dvh-2.5rem))] sm:max-h-[min(720px,calc(100dvh-2.5rem))] sm:max-w-[420px] sm:rounded-2xl sm:border sm:border-zinc-200/90 sm:shadow-2xl dark:border-zinc-700/80 dark:bg-zinc-900 dark:sm:ring-1 dark:sm:ring-zinc-700/80"
                        >
                                        ...
                                        >
                                            {msg.sender === 'ai' ? (
                                                ...
                                                <div className="mx-auto flex w-full max-w-3xl justify-end px-4 py-3">
                                                    <div className="flex max-w-[min(100%,20rem)] items-end gap-2">
                                                        <div className="min-w-0 rounded-2xl bg-zinc-200 px-3.5 py-2.5 text-[15px] leading-7 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100">
                                                            <p className="whitespace-pre-wrap break-words text-left">{msg.text}</p>
                                                            <p className="mt-1 text-right text-[10px] text-zinc-500 dark:text-zinc-500">
                                                                {formatTime(msg.timestamp)}
                                                            </p>
                                                        </div>
                                                        <div className="mb-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-sm bg-zinc-300/80 dark:bg-zinc-700">
                            ...
                            <div className="flex-shrink-0 border-t border-zinc-200/90 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900">
                                <div className="mx-auto flex max-w-3xl items-end gap-1.5 rounded-2xl border border-zinc-200/90 bg-zinc-50/90 pl-1 pr-1.5 py-1.5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800/80 sm:gap-2">
                                    {ragEligible === true && (
                                        <>
                                            <input
                                                ref={fileInputRef}
                                            ...
                                            <button
                                                ...
                                                onClick={onPickFile}
                                                disabled={uploadBusy}
                                                className="mb-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl text-zinc-600 transition hover:bg-zinc-200/80 disabled:opacity-50 dark:text-zinc-300 dark:hover:bg-zinc-700/80"
                                    ...
                                    <textarea
                                        ...
                                        className="min-h-[44px] min-w-0 max-h-40 flex-1 resize-none bg-transparent px-1 py-2.5 text-[15px] leading-5 text-zinc-900 placeholder:text-zinc-400 focus:outline-none dark:text-zinc-100 dark:placeholder:text-zinc-500"
                                    />
                                    <button
                                        type="button"
                                        onClick={handleSend}
