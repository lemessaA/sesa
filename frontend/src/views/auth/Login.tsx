...
const PAGE_TITLE = 'Sign in â€” SESA TECHNOLOGY';
const PAGE_DESCRIPTION =
    'SESA TECHNOLOGY â€” Safe Educational & Skill Academy. Sign in or create a learner account.';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data?.code === 'DB_OFFLINE' || axiosErr.response?.status === 503) {
                setError(
                    `${baseMsg} If you manage this server: open MongoDB Atlas â†’ Network Access, allow your IP (or 0.0.0.0/0 for testing), confirm the cluster is running, check MONGO_URI in backend/.env, then restart the API.`
                );
            } else {
                setError(baseMsg);
            }
